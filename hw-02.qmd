...
pac <- pac |>
  extract(
    ...
    into = "last_year",
    regex = "-([0-9]{4})"
  )  |> mutate(last_year=as.integer(last_year))
...
```{r}
ggplot(uk, aes(x=last_year, y=total, color=party)) + 
...
ggplot(df, aes(x=col1, y=col2)) +
  geom_circle(aes(x0=50, y0=50, r=50), fill="#D53A2A", color="#D53A2A", inherit.aes=FALSE)+
  geom_circle(aes(x0=50, y0=50, r=35), fill="white", color="white", inherit.aes=FALSE)+
  geom_circle(aes(x0=50, y0=50, r=15), fill="#D53A2A", color="#D53A2A", inherit.aes=FALSE)+
 ...
 coord_fixed(clip="off") + 
  ...
  annotate("text", label="TARGET", x=50, y=0, vjust=2, color="#D53A2A", size=12, fontface="bold") +
    geom_circle(aes(x0=110, y0=-38, r=6), fill="white", color="#D53A2A", inherit.aes=FALSE, linewidth=1) + 
    annotate("text", label="R", x=110, y=0, vjust=6.44, color="#D53A2A", size=4, fontface="bold")
